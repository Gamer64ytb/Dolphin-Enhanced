...
void Arm64GPRCache::FlushRegister(size_t index, bool maintain_state, ARM64Reg tmp_reg)
{
  ...
  else if (reg.GetType() == RegType::Immediate)
  {
    if (!reg.GetImm())
    {
      m_emit->STR(IndexType::Unsigned, bitsize == 64 ? ARM64Reg::ZR : ARM64Reg::WZR,
                  PPC_REG, guest_reg.ppc_offset);
    }
    else
    {
...
void Arm64GPRCache::Flush(FlushMode mode, ARM64Reg tmp_reg)
{
  ...
  {
    bool flush = true;
    if (m_guest_registers[i].GetType() == RegType::Register)
    {
  ...
  FlushRegisters(to_flush, mode == FlushMode::MaintainState, tmp_reg);
