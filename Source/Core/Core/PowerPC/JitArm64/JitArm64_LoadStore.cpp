...
void JitArm64::SafeStoreFromReg(s32 dest, u32 value, s32 regOffset, u32 flags, s32 offset)
{
  ...
  if (is_immediate && jo.optimizeGatherPipe && PowerPC::IsOptimizableGatherPipeWrite(imm_addr))
  {
    ...
    LDR(INDEX_UNSIGNED, ARM64Reg::X0, PPC_REG, PPCSTATE_OFF(gather_pipe_ptr));
    if (accessSize == 32)
    {
      REV32(ARM64Reg::W1, RS);
      STR(INDEX_POST, ARM64Reg::W1, ARM64Reg::X0, 4);
    }
    else if (accessSize == 16)
    {
      REV16(ARM64Reg::W1, RS);
      STRH(INDEX_POST, ARM64Reg::W1, ARM64Reg::X0, 2);
    }
    else
    {
      STRB(INDEX_POST, RS, ARM64Reg::X0, 1);
    }
    STR(INDEX_UNSIGNED, ARM64Reg::X0, PPC_REG, PPCSTATE_OFF(gather_pipe_ptr));
...
void JitArm64::dcbz(UGeckoInstruction inst)
{
  ...
  EmitBackpatchRoutine(BackPatchInfo::FLAG_ZERO_256, true, true, ARM64Reg::W0, EncodeRegTo64(addr_reg),
