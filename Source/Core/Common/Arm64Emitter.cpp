                                         ...
                                         ARM64Reg Rm, ArithOption Option)
{
  ...
  Write32((b64Bit << 31) | (flags << 29) | (ArithEnc[instenc] << 21) |
          (Option.GetType() == ArithOption::TYPE_EXTENDEDREG ? (1 << 21) : 0) | (DecodeReg(Rm) << 16) |
          Option.GetData() | (DecodeReg(Rn) << 5) | DecodeReg(Rd));
}
                                             ...
                                             CCFlags cond)
{
  ...
  Write32((b64Bit << 31) | (op << 30) | (1 << 29) | (0xD2 << 21) | (DecodeReg(Rm) << 16) | (cond << 12) |
          (DecodeReg(Rn) << 5) | nzcv);
}
                                         ...
                                         CCFlags cond)
{
  ...
  Write32((b64Bit << 31) | (CondSelectEnc[instenc][0] << 30) | (0xD4 << 21) | (DecodeReg(Rm) << 16) |
          (cond << 12) | (CondSelectEnc[instenc][1] << 10) | (DecodeReg(Rn) << 5) | DecodeReg(Rd));
}
                                       ...
                                       ARM64Reg Ra)
{
  ...
  Write32((b64Bit << 31) | (0xD8 << 21) | (Data3SrcEnc[instenc][0] << 21) | (DecodeReg(Rm) << 16) |
          (Data3SrcEnc[instenc][1] << 15) | (DecodeReg(Ra) << 10) | (DecodeReg(Rn) << 5) | DecodeReg(Rd));
}
...
void ARM64XEmitter::EncodeLoadStoreIndexedInst(u32 op, ARM64Reg Rt, ARM64Reg Rn, s32 imm, u8 size)
{
  ...
  Write32((b64Bit << 30) | (op << 22) | (bVec << 26) | (imm << 10) |
          (DecodeReg(Rn) << 5) | DecodeReg(Rt));
}
...
void ARM64XEmitter::EncodeMOVWideInst(u32 op, ARM64Reg Rd, u32 imm, ShiftAmount pos)
{
  ...
  Write32((b64Bit << 31) | (op << 29) | (0x25 << 23) | (pos << 21) | (imm << 5) | DecodeReg(Rd));
}
...
void ARM64XEmitter::SetJumpTarget(FixupBranch const& branch)
{
  ...
  case 0:  // CBZ
  {
    ...
    inst = (b64Bit << 31) | (0x1A << 25) | (Not << 24) | (MaskImm19(distance) << 5) | DecodeReg(branch.reg);
  }
  break;
...
{
  EncodeSystemInst(0, 3, 2, 0, op, ARM64Reg::WSP);
}
...
{
  EncodeSystemInst(0, 3, 3, type, 4, ARM64Reg::WSP);
}
void ARM64XEmitter::DMB(BarrierType type)
{
  EncodeSystemInst(0, 3, 3, type, 5, ARM64Reg::WSP);
}
void ARM64XEmitter::ISB(BarrierType type)
{
  EncodeSystemInst(0, 3, 3, type, 6, ARM64Reg::WSP);
}
...
void ARM64XEmitter::MOV(ARM64Reg Rd, ARM64Reg Rm)
{
  if (IsGPR(Rd) && IsGPR(Rm))
    ORR(Rd, Is64Bit(Rd) ? ARM64Reg::ZR : ARM64Reg::WZR, Rm, ArithOption(Rm, ST_LSL, 0));
  else
    ASSERT_MSG(DYNA_REC, false, "Non-GPRs not supported in MOV");
}
void ARM64XEmitter::MVN(ARM64Reg Rd, ARM64Reg Rm)
{
  ORN(Rd, Is64Bit(Rd) ? ARM64Reg::ZR : ARM64Reg::WZR, Rm, ArithOption(Rm, ST_LSL, 0));
}
...
void ARM64XEmitter::MOVI2R(ARM64Reg Rd, u64 imm)
{
      ...
      (!Is64Bit(Rd) && imm == std::numeric_limits<u32>::max()))
  {
    ...
    // Set to ~ZR
    ARM64Reg ZR = Is64Bit(Rd) ? ARM64Reg::SP : ARM64Reg::WSP;
...
void ARM64XEmitter::ABI_PushRegisters(BitSet32 registers)
{
  ...
  if (num_regs & 1)
  {
    STR(INDEX_PRE, ARM64Reg::X0 + *it++, ARM64Reg::SP, -stack_size);
  }
  else
  {
    ARM64Reg first_reg = ARM64Reg::X0 + *it++;
    ARM64Reg second_reg = ARM64Reg::X0 + *it++;
    STP(INDEX_PRE, first_reg, second_reg, ARM64Reg::SP, -stack_size);
  }
  ...
  for (int i = 0; i < (num_regs - 1) / 2; i++)
  {
    ...
    STP(INDEX_SIGNED, odd_reg, even_reg, ARM64Reg::SP, 16 * (i + 1));
  }
...
void ARM64XEmitter::ABI_PopRegisters(BitSet32 registers, BitSet32 ignore_mask)
{
  ...
  for (int i = 0; i < (num_regs - 1) / 2; i++)
  {
    ARM64Reg odd_reg = ARM64Reg::X0 + *it++;
    ARM64Reg even_reg = ARM64Reg::X0 + *it++;
    LDP(INDEX_SIGNED, odd_reg, even_reg, ARM64Reg::SP, 16 * (i + 1));
  }

  // Post loading the first (two) registers.
  if (num_regs & 1)
    LDR(INDEX_POST, first, ARM64Reg::SP, stack_size);
  else
    LDP(INDEX_POST, first, second, ARM64Reg::SP, stack_size);
                                          ...
                                          ARM64Reg Rn, ARM64Reg Rm)
{
  ...
  Write32((M << 31) | (S << 29) | (0b11110001 << 21) | (type << 22) | (DecodeReg(Rm) << 16) | (opcode << 12) |
                                                     ...
                                                     ARM64Reg Rt, ARM64Reg Rn, ARM64Reg Rm)
{
  ...
  Write32((quad << 30) | (0x1B << 23) | (L << 22) | (R << 21) | (DecodeReg(Rm) << 16) | (opcode << 13) |
          (S << 12) | (size << 10) | (DecodeReg(Rn) << 5) | DecodeReg(Rt));
}
...
void ARM64FloatEmitter::EmitCompare(bool M, bool S, u32 op, u32 opcode2, ARM64Reg Rn, ARM64Reg Rm)
{
  ...
  Write32((M << 31) | (S << 29) | (0xF1 << 21) | (is_double << 22) | (DecodeReg(Rm) << 16) | (op << 14) |
          (1 << 13) | (DecodeReg(Rn) << 5) | opcode2);
}
                                       ...
                                       ARM64Reg Rm)
{
  ...
  Write32((M << 31) | (S << 29) | (0xF1 << 21) | (is_double << 22) | (DecodeReg(Rm) << 16) | (cond << 12) |
          (3 << 10) | (DecodeReg(Rn) << 5) | DecodeReg(Rd));
}
...
void ARM64FloatEmitter::EmitPermute(u32 size, u32 op, ARM64Reg Rd, ARM64Reg Rn, ARM64Reg Rm)
{
  ...
  Write32((quad << 30) | (7 << 25) | (encoded_size << 22) | (DecodeReg(Rm) << 16) | (op << 12) | (1 << 11) |
          (DecodeReg(Rn) << 5) | DecodeReg(Rd));
}
                                                       ...
                                                       ARM64Reg Rn)
{
  ...
  Write32((quad << 30) | (3 << 26) | (L << 22) | (opcode << 12) | (encoded_size << 10) | (DecodeReg(Rn) << 5) |
          DecodeReg(Rt));
}
                                           ...
                                           ARM64Reg Rd, ARM64Reg Rn, ARM64Reg Rm)
{
  ...
  Write32((quad << 30) | (U << 29) | (0xF << 24) | (size << 22) | (L << 21) | (DecodeReg(Rm) << 16) |
          (opcode << 12) | (H << 11) | (DecodeReg(Rn) << 5) | DecodeReg(Rd));
}
...
void ARM64FloatEmitter::UCVTF(ARM64Reg Rd, ARM64Reg Rn)
{
  if (IsScalar(Rn))
  {
    ...
    bool sign = true;
    int sz = IsDouble(Rn);
    Write32((0x5e << 24) | (sign << 29) | (sz << 22) | (0x876 << 10) | (DecodeReg(Rn) << 5) | DecodeReg(Rd));
  }
  else
  {
    bool sf = Is64Bit(Rn);
...
void ARM64FloatEmitter::ABI_PushRegisters(BitSet32 registers, ARM64Reg tmp)
{
  ...
  else
  {
    ...
    for (auto it : registers)
    {
      ...
      if (pair_regs.size() == 2)
      {
        STP(128, INDEX_PRE, pair_regs[0], pair_regs[1], ARM64Reg::SP, -32);
        pair_regs.clear();
      }
    }
    if (pair_regs.size())
      STR(128, INDEX_PRE, pair_regs[0], ARM64Reg::SP, -16);
  }
}
  ...
  if (bundled_loadstore && tmp != ARM64Reg::INVALID_REG)
  {
    ...
    for (int i = 0; i < 32; ++i)
    {
      ...
      if (count == 1)
        ...
        LD1(64, count, INDEX_POST, ARM64Reg::Q0 + i, ARM64Reg::SP);
    ...
    for (auto& it : island_regs)
    {
      ...
      if (pair_regs.size() == 2)
      {
        LDP(128, INDEX_POST, pair_regs[0], pair_regs[1], ARM64Reg::SP, 32);
        pair_regs.clear();
      }
    }
    if (pair_regs.size())
      LDR(128, INDEX_POST, pair_regs[0], ARM64Reg::SP, 16);
  }
  else
  {
    bool odd = num_regs % 2;
    ...
    for (int i = 31; i >= 0; --i)
    {
      ...
      if (odd)
      {
        ...
        odd = false;
        LDR(128, INDEX_POST, ARM64Reg::Q0 + i, ARM64Reg::SP, 16);
      }
      else
      {
        ...
        if (pair_regs.size() == 2)
        {
          LDP(128, INDEX_POST, pair_regs[1], pair_regs[0], ARM64Reg::SP, 32);
          pair_regs.clear();
        }
      }
    }
...
void ARM64XEmitter::ANDSI2R(ARM64Reg Rd, ARM64Reg Rn, u64 imm, ARM64Reg scratch)
{
  ...
  {
    ANDS(Rd, Rn, Is64Bit(Rn) ? ARM64Reg::ZR : ARM64Reg::WZR, ArithOption(Rd, ST_LSL, 0));
  }
  else
  {
    ASSERT_MSG(DYNA_REC, scratch != ARM64Reg::INVALID_REG,
